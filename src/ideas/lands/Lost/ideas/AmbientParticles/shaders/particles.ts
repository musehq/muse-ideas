...
export const vert = `
  ...
  #define radiusVariationAmplitude 0.2
  #define radius 0.35
  ...
  void main() {
      ...
      vec3 transformed = position.xyz * 0.1;
  ...
  useMemo(
    () =>
      new ShaderMaterial({
        ...
        defines: {
          fogEnabled: `${fog}`,
          fogColor: `vec3(${new Color(color).toArray()})`,
          fogFar: fogDistance.toFixed(1),
        },
        vertexShader: vert,
        fragmentShader: frag,
      }),
    [frag, vert, uniforms, fog, color, fogDistance]
  );
