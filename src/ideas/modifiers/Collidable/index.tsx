...
export default function Collidable(props: CollidableProps) {
  ...
  useEffect(() => {
    ...
    if (colliderMeshes) {
      ...
      console.log("found " + colliderMeshes.length + " named collider meshes");
      return;
    }
    ...
    if (triCount < triLimit) {
      setCollisionGeos(geos);
      console.log("found " + meshes.length + " meshes, using raw");
    } else {
      const perc = triLimit / triCount;
      console.log("found " + meshes.length + " meshes, using bvh");
      console.log("generating bvh at ", (perc * 100).toFixed(2) + "%");
      const bvhGeos = geos
